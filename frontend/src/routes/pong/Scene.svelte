<script lang="ts">
	import * as Threlte from '@threlte/core'
	import { DEG2RAD } from 'three/src/math/MathUtils.js'
	import { Text } from '@threlte/extras'
	import { useGltf } from '@threlte/extras'

	import { user, rival, userName, rivalName } from './store';

	const setText = (): string => {
		if ($rival && $user)
			return $rivalName + "\n" + $rival.points.toString() + "\n\n\n\n\n\n" + $userName + "\n" + $user.points.toString();
		else if ($rival && !$user)
			return $rivalName + "\n" + $rival.points.toString() + "\n\n\n\n\n\n" + "UserName\n--";
		else if (!$rival && $user)
			return "RivalName\n--" + "\n\n\n\n\n" + $userName + "\n" + $user.points.toString();
		else
			return "RivalName\n--" + "\n\n\n\n\n" + "UserName\n--";
	}

	let PointText: string = "";
	Threlte.useFrame(() =>{
		PointText = setText();
	});

</script>

<!-- WALL RIGHT -->
<Threlte.T.Group
	position={[16, 1, 0]}
>
	<Threlte.T.Mesh position={[0, 1, 0]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.CylinderGeometry args={[0.2, 0.2, 31]}/>
		<Threlte.T.MeshBasicMaterial color="#ffffff"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, 0, 0]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.CylinderGeometry args={[0.2, 0.2, 31]}/>
		<Threlte.T.MeshBasicMaterial color="#ffffff"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, -1, 0]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.CylinderGeometry args={[0.2, 0.2, 31]}/>
		<Threlte.T.MeshBasicMaterial color="#ffffff"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, 0, 15.5]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.BoxGeometry args={[1, 1, 3]}/>
		<Threlte.T.MeshStandardMaterial color="#000000"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, 0, -15.5]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.BoxGeometry args={[1, 1, 3]}/>
		<Threlte.T.MeshStandardMaterial color="#000000"/>
	</Threlte.T.Mesh>
</Threlte.T.Group>

<!-- WALL LEFT -->
<Threlte.T.Group
	position={[-15, 1, 0]}
>
	<Threlte.T.Mesh position={[0, 1, 0]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.CylinderGeometry args={[0.2, 0.2, 31]}/>
		<Threlte.T.MeshBasicMaterial color="#ffffff"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, 0, 0]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.CylinderGeometry args={[0.2, 0.2, 31]}/>
		<Threlte.T.MeshBasicMaterial color="#ffffff"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, -1, 0]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.CylinderGeometry args={[0.2, 0.2, 31]}/>
		<Threlte.T.MeshBasicMaterial color="#ffffff"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, 0, 15.5]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.BoxGeometry args={[1, 1, 3]}/>
		<Threlte.T.MeshStandardMaterial color="#000000"/>
	</Threlte.T.Mesh>
	<Threlte.T.Mesh position={[0, 0, -15.5]} rotation={[DEG2RAD * 90, 0, 0]}>
		<Threlte.T.BoxGeometry args={[1, 1, 3]}/>
		<Threlte.T.MeshStandardMaterial color="#000000"/>
	</Threlte.T.Mesh>
</Threlte.T.Group>
	

<!-- Ground -->
<!-- <T.Mesh position={[0, 0, 0]} rotation.x={DEG2RAD * 90 } receiveShadow>
	<T.PlaneGeometry args={[20, 30]} />
	<T.MeshStandardMaterial color="#ffffff" side={Three.DoubleSide} />
</T.Mesh> -->

<Threlte.T.Group
	rotation={[DEG2RAD * -90, 0, 0]}
	position={[0, -1.9, 12]}
>
	<Text
		text={PointText}
		color="white"
		fontSize={2}
		anchorX="50%"
		anchorY="100%"
		textAlign="center"
	/>
</Threlte.T.Group>

<!-- SKYSCRAPER -->

{#await useGltf('/futuristic-tower/source/tower.glb') then tower}
	<Threlte.T is={tower.scene} position={[0, -60, 0]} scale={1} />
{/await}
<!-- {#await useGltf('/city.glb') then city}
	<T is={city.scene} position={[40, -40, -40]} scale={[1, 1, 2]} />
{/await} -->
<!-- {#await useGltf('/oil_build.glb') then oil}
	<T is={oil.scene} position={[-40, -100, -40]} scale={0.3} />
{/await}
{#await useGltf('/build01.glb') then build01}
	<T is={build01.scene} position={[-35, -100, 0]} scale={10} />
{/await}
{#await useGltf('/city.glb') then city}
	<T is={city.scene} position={[-120, -100, 70]} scale={2} />
{/await} -->

