...
class GameConsumer(AsyncWebsocketConsumer):
    games = []
    #user_model = TranscendenceUser
    #match_model = MatchEntry
        ...
        for game in self.games:
            if game.room == data["room"]:
                ...
                if game.player1.points >= 7 or game.player2.points >= 7:
                    ...
                    try:
                        winner = await sync_to_async(TranscendenceUser.objects.get)(username=winner_player.name)
                        loser = await sync_to_async(TranscendenceUser.objects.get)(username=loser_player.name)
